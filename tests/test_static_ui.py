...
class StaticUiContracts(unittest.TestCase):
    ...
    def test_backlog_is_a_primary_collection_and_session_prep_is_opt_in(self):
        ...
        self.assertIn('envSel.value = backlogIdea.environment', self.app)
        self.assertIn('"SERVER · hostagent" : "PROJECT · devagent"', self.app)
        backend = (ROOT / "app/main.py").read_text(encoding="utf-8")
        self.assertIn("CREATE TABLE IF NOT EXISTS backlog_ideas", backend)
        self.assertIn("ALTER TABLE backlog_ideas ADD COLUMN environment", backend)
        self.assertIn('metadata["environment"]', backend)
